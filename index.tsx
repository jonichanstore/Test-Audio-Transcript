...
class VoiceNotesApp {
  ...
  private async handleFileUpload(event: Event): Promise<void> {
    ...
    if (input.files && input.files.length > 0) {
      ...
      try {
        // Use file.type, or provide a sensible default like 'audio/mpeg' if it's empty.
        // Most browsers will set file.type correctly for known audio types.
        const mimeType = file.type || 'application/octet-stream'; // Fallback, Gemini might infer
        if (!mimeType.startsWith('audio/')) {
            console.warn(`Uploaded file MIME type "${mimeType}" might not be an audio file. Proceeding anyway.`);
        }
        await this.processAudio(file, mimeType);
      } catch (error) {
        console.error('Error processing uploaded file:', error);
        this.recordingStatus.textContent = 'Error processing file. Please try a different audio file.';
        this.polishedNote.innerHTML = `<p><em>Could not process the uploaded file: ${error instanceof Error ? error.message : String(error)}</em></p>`;
  ...
  private async startRecording(): Promise<void> {
    try {
      // Reset UI for new content, including file input
      this.createNewNote();
      ...
      this.mediaRecorder.onstop = () => {
        ...
        if (this.audioChunks.length > 0) {
          ...
          const mimeTypeForProcessing = this.mediaRecorder?.mimeType || audioBlob.type || 'audio/webm';
          this.processAudio(audioBlob, mimeTypeForProcessing).catch((err) => {
  ...
  private async stopRecording(): Promise<void> {
    if (this.mediaRecorder && this.isRecording) {
      try {
        this.mediaRecorder.stop(); // This will trigger 'onstop' which calls processAudio
      } catch (e) {
        console.error('Error stopping MediaRecorder:', e);
        this.stopLiveDisplay(); // Ensure UI resets if stop itself fails
      }
  ...
  private async processAudio(audioBlob: Blob, mimeTypeToUse: string): Promise<void> {
    if (audioBlob.size === 0) {
      this.recordingStatus.textContent =
        'No audio data captured or file is empty. Please try again.';
      return;
    }
    ...
    try {
      ...
      await this.getTranscription(base64Audio, mimeTypeToUse);
    } catch (error) {
      console.error('Error in processAudio:', error);
      this.recordingStatus.textContent =
        'Error processing audio. Please try again.';
      this.polishedNote.innerHTML = `<p><em>Error processing audio: ${error instanceof Error ? error.message : String(error)}</em></p>`;
       this.polishedNote.classList.remove('placeholder-active');
    }
  }
  ...
  private async getPolishedNote(): Promise<void> {
    try {
      if (
        ...
        this.polishedNote.innerHTML = placeholder; // Use innerHTML for placeholder too
      ...
      if (polishedText) {
        const parsedOutput = marked.parse(polishedText);
        const htmlContent = typeof parsedOutput === 'string' ? parsedOutput : await parsedOutput;
  ...
  private createNewNote(): void {
    ...
    if (this.isRecording) {
      this.mediaRecorder?.stop(); // This will trigger onstop, then stopLiveDisplay
      this.isRecording = false;
      this.recordButton.classList.remove('recording');
      this.recordButton.setAttribute('title', 'Start Recording');
      // stopLiveDisplay is called in mediaRecorder.onstop, or here if it was already stopped.
    } else {
       this.stopLiveDisplay(); // Ensure live display is stopped if not actively recording
    }
  }
}
  ...
  document
    ...
    .forEach((el) => {
      ...
      function updatePlaceholderState() {
        const currentText = (
          el.id === 'polishedNote' ? el.innerText : el.textContent // Use innerText for polishedNote to better reflect visible content
        )?.trim();

        if (currentText === '' || currentText === placeholder) {
          if (el.id === 'polishedNote' && (el.innerHTML === '' || el.innerHTML === '<br>')) { // Check innerHTML for polishedNote
            el.innerHTML = placeholder;
          } else if (el.id !== 'polishedNote' && el.textContent === '') {
      ...
      el.addEventListener('focus', function () {
        ...
        if (currentText === placeholder) {
          if (this.id === 'polishedNote') this.innerHTML = ''; // Clear with innerHTML
      ...
      // For polishedNote, also listen to input to handle complex HTML changes
      if (el.id === 'polishedNote') {
        el.addEventListener('input', function() {
            // Basic check: if it's empty after input, re-apply placeholder logic on blur
            if (this.innerText.trim() === '') {
                // Placeholder will be handled by blur
            } else {
                this.classList.remove('placeholder-active');
            }
        });
